...
namespace Consultorio.Presentacion1
{
    public partial class frmGestionTurnos : Form
    {
        ...
        {
            // Evitar que se ejecute cuando todavía se está llenando el combo
            if (cmbProfesional.SelectedValue == null || !(cmbProfesional.SelectedValue is int))
                return;

            // Cargar agenda solo cuando hay un profesional válido seleccionado
            CargarAgenda();
        }
        ...
        private void CargarProfesionales(int idEspecialidad)
        {
            try
            {
                var listaProfesionales = profesionalBLL.ListarPorEspecialidad(idEspecialidad);

                // 🔹 Primero configurás los nombres de las propiedades
                cmbProfesional.DisplayMember = "NombreCompleto";
                cmbProfesional.ValueMember = "ID_Profesional";

                // 🔹 Recién después asignás la lista
            ...
            {
                MessageBox.Show(ex.Message, "Error al cargar profesionales", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void CargarAgenda()
        {
            if (cmbProfesional.SelectedValue == null)
                return;
        ...
        {
            txtNombreCompleto.Text = p.Nombre;
